...
function initManager(){
    $.ajax({
        ...
        success: function (data) {
            if (data.length > 0) {
                if(data.length>1){
                    data.splice(0,0,{"userName":"全部","userNo":""});//增加全部选项 #
                }
                //添加option选项
                for(var i=0;i<data.length;i++){
...
var initDataTable = function(){
    ...
    $("#resultGrid").on('xhr.dt', function ( e, settings, json, xhr ) {
        ...
        ajax : {
            "url" : "/erp/ManagerMonthlyController/getMonthlyScore",
            "method": "POST",
            "data" : function(){
                return {
                    "orgNo":$('#areaQ').val(),
                    "distributeNo":$('#operateCenterQ').val(),
                    "managerNo":$('#managerQ').val(),
...
function exportLimit(){
    jQuery.ajax({
        ...
        dataType: 'json',
        cache: false,
        url: "/erp/ManagerMonthlyController/exportLimit",
        data: {
            "orgNo":$('#areaQ').val(),
            "distributeNo":$('#operateCenterQ').val(),
            "managerNo":$('#managerQ').val(),
